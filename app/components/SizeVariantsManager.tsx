...
export default function SizeVariantsManager({ 
  ...
  return (
    <div className="mb-6">
      {error && (
        <div className="mb-4 p-3 text-red-600 bg-red-100 rounded-md border border-red-200 flex justify-between items-center">
          <span>{error}</span>
          <button 
            onClick={() => setError('')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="md:col-span-1 border border-gray-200 rounded-lg bg-white">
              ...
              <div className="space-y-1">
                {sizeVariants.map((variant, index) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-md flex justify-between items-center ${activeSizeIndex === index ? 'bg-green-50 text-green-700 font-medium' : 'hover:bg-gray-50'}`}
                  >
        ...
        <div className="md:col-span-3">
          {sizeVariants.length === 0 ? (
            <div className="text-center py-8 border border-dashed border-gray-300 rounded-lg bg-gray-50 h-full flex flex-col items-center justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-gray-400 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
              <p className="text-gray-500 italic">Add a size first to manage color variants</p>
            </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {COMMON_COLORS.map(color => (
                      <button
                        ...
                        className={`w-8 h-8 rounded-full border-2 ${colorInput === color.name ? 'border-blue-500 ring-2 ring-blue-300' : 'border-gray-300'}`}
                    ...
                    <button
                      ...
                      className={`w-8 h-8 rounded-full border-2 flex items-center justify-center ${customColorMode ? 'border-blue-500 bg-gray-100' : 'border-gray-300 bg-white'}`}
                      title="Custom color"
                    >
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {sizeVariants[activeSizeIndex].colorVariants.map((colorVariant, colorIndex) => (
                          <tr key={`${sizeVariants[activeSizeIndex].size}-${colorVariant.color}`} className="hover:bg-gray-50">
            ...
            <div className="text-center py-8 border border-dashed border-gray-300 rounded-lg bg-gray-50">
              <p className="text-gray-500 italic">Select a size to manage its color variants</p>
            </div>
          )}
        </div>
      </div>
